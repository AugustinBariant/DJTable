...
public class SurfaceInputs : MonoBehaviour
{
    ...
    {
        foreach (KeyValuePair<int, float> entry in removalTimes) {
            if (Time.time - entry.Value >= 0.2f) {
                lastRemovedObjects.Add(surfaceObjects[entry.Key]);
                surfaceObjects.Remove(entry.Key);
            } 
        }
    }
